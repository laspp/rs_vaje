...
class PrivateL1L2Hierarchy(AbstractClassicCacheHierarchy):
    ...
    def incorporate_cache(self, board):
        ...
        self.l1d_cache = myCustomCache(size=self._l1d_size, assoc=self._l1d_assoc, tag_latency=1, data_latency=1, response_latency=1, mshrs=16);
        # 1b. create the L1I cache
        self.l1i_cache = myCustomCache(size=self._l1i_size, assoc=self._l1i_assoc, tag_latency=1, data_latency=1, response_latency=1, mshrs=16);
        ...
        self.l2_cache = myCustomCache(size=self._l2_size, assoc=self._l2_assoc, tag_latency=10, data_latency=10, response_latency=10, mshrs=16);
        # 2b. create the L2 xbar
        self.L2_xbar = L2XBar();
        # 2c. connect the L1D and L1I caches to the L2 cache
        self.L2_xbar.cpu_side_ports = self.l1d_cache.mem_side
        self.L2_xbar.cpu_side_ports = self.l1i_cache.mem_side

        # 2d. connect the L2 cache to the L2 xbar
        self.L2_xbar.mem_side_ports = self.l2_cache.cpu_side
